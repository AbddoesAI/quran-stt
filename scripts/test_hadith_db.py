...
if result:
    print(f"TEST 1 PASS: {result.collection} #{result.hadith_number} conf={result.confidence:.2f} paraphrase={result.is_paraphrase}")
else:
...
if result2:
    print(f"TEST 2 PASS: {result2.collection} #{result2.hadith_number} conf={result2.confidence:.2f}")
else:
...
print(f"TEST 3 {'PASS' if result3 is None else 'FAIL'}: Short text correctly rejected" if result3 is None else f"TEST 3 FAIL: Short text matched")

# Test 4: Partial hadith
result4 = matcher.match("من حسن إسلام المرء تركه ما لا يعنيه")
if result4:
    print(f"TEST 4 PASS: {result4.collection} #{result4.hadith_number} conf={result4.confidence:.2f} paraphrase={result4.is_paraphrase}")
else:
